		...
		//		$this->db->order_by("lpid", "desc"); 
		    $result = $this->db->get();
			  ...
			  $sql="SELECT  
					...
					,(select n_value FROM vw_sales_despatch_transaction_calss_fnd_flex_values_vl_part_tanker j WHERE j.n_value_id = lp.product_type_id ) as n_value
					,(select n_value_id FROM vw_sales_despatch_transaction_calss_fnd_flex_values_vl_part_tanker j WHERE j.n_value_id = lp.product_type_id ) as prod_type_id
	...
	function save_daily_details($dcprodinserts)
	{
		  ...
		  function check_prodnameduplicates_lead($prodid,$customerid)
		{
 			...
 			//   $sql= "SELECT dct_itemgroup from dailycall_dtl WHERE dct_customergroup ='".$prodname."'  AND dct_itemgroup ='".$prodname."'";
 				$sql="SELECT leaddetails.leadid FROM leaddetails,leadproducts WHERE leaddetails.company=".$customerid." AND leadproducts.productid::TEXT='".$prodid."'";
